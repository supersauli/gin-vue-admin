...
test.describe('用户管理', () => {
  ...
  test('创建用户流程', async ({ page }) => {
    await page.route('**/admin/users/create', async route => {
      const data = JSON.parse(route.request().postData() || '{}')
      const timestamp = Date.now()
      if (!data.openId) data.openId = `e2e_test_${timestamp}`
      if (!data.nickName) data.nickName = `E2E用户-${timestamp}`
    ...
    const timestamp = Date.now()
    await page.locator('.el-dialog input[placeholder="请输入OpenID"]').fill(`e2e_test_${timestamp}`)
    await page.locator('.el-dialog input[placeholder="请输入昵称"]').fill(`E2E用户-${timestamp}`)
