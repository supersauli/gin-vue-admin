...
test.describe('NPC管理', () => {
  test.beforeEach(async ({ page }) => {
    await loginAndNavigate(page, 'NPC管理')
  })

  test('页面标题正确', async ({ page }) => {
    await expect(page.locator('h2').filter({ hasText: 'NPC管理' })).toBeVisible()
  })
  ...
  test('创建NPC流程', async ({ page }) => {
    await page.route('**/admin/npc/create', async route => {
      const data = JSON.parse(route.request().postData() || '{}')
      if (!data.npcId) data.npcId = Date.now()
      if (!data.isActive) data.isActive = true
      if (!data.level) data.level = 1
      if (!data.attributes || data.attributes === '') data.attributes = '{}'
      if (!data.skills || data.skills === '') data.skills = '[]'
      await route.continue({ postData: JSON.stringify(data) })
    })
    
    await page.locator('button:has-text("创")').click()
    await expect(page.locator('.el-dialog__title')).toContainText('创建NPC')
    
    const timestamp = Date.now()
    await page.locator('.el-dialog input[placeholder="请输入NPC名称"]').fill(`E2E-NPC-${timestamp}`)
    await page.locator('.el-dialog input[placeholder="请输入NPC类型"]').fill('商人')
    ...
    await expect(page.locator(`text=E2E-NPC-${timestamp}`)).toBeVisible({ timeout: 10000 })
  })
})
